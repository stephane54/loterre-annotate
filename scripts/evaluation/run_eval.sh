...
python3 "$CLI" --text ./examples/texts/P66_en.jsonl --dict-id P66_en --silent > "$OUTDIR/P66_en_pred.json"
...
python3 "$CLI" --text ./examples/texts/9SD_en.jsonl --dict-id 9SD_en --silent > "$OUTDIR/9SD_en_pred.json"
python3 "$EVAL_PY" --gold ./gold_cleaned/gold_9SD_en.jsonl --pred "$OUTDIR/9SD_en_pred.json" --out-json "$OUTDIR/9SD_en_eval.json"

# ── Français — gold inline (examples/texts/*_fr.jsonl) ───────────────────────
...
for vocab in P66_fr 27X_fr 9SD_fr 8HQ_fr B9M_fr BVM_fr QX8_fr; do
  ...
  GOLD="./examples/texts/${vocab}.jsonl"
