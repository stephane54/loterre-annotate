...
cat data/jsonl/P66_en.jsonl | python3 "$CLI" \
  --dict-id P66_en \
  --silent \
...
cat data/jsonl/9SD_en.jsonl | python3 "$CLI" \
  --dict-id 9SD_en \
  --silent \
...
printf '%s\n' '{"id":"bruit1","value":"et ou ni mais"}' '{"id":"bruit2","value":"il elle on bien"}' | python3 "$CLI" \
  --dict-id P66_fr \
  --silent \
