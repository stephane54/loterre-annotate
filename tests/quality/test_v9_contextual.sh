...
printf '%s\n' '{"id":"ctx1","value":"The word Parthenon comes from the Greek parthénos as well as other forms."}' | python3 "$CLI" annotate \
  --dict-id 9SD_en \
  --silent > "$OUTDIR/well_context.json"

echo
echo "== Test 2: weak function words =="
printf '%s\n' '{"id":"ctx2","value":"and or it are only function words here."}' | python3 "$CLI" annotate \
  --dict-id 9SD_en \
  --silent > "$OUTDIR/function_words.json"

echo
echo "== Test 3: lexical entity context =="
printf '%s\n' '{"id":"ctx3","value":"Italy and Rome are mentioned here."}' | python3 "$CLI" annotate \
  --dict-id 9SD_en \
